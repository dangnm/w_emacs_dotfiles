...
** Auto complete config
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package company               
       ...
       (with-eval-after-load "evil"
         (define-key evil-insert-state-map (kbd "C-f") 'company-show-location)
         )
       )
   #+END_SRC
...
*** rope (code navigation & completion)
    ...
    - Enable it by using ~M-x robe-mode~ and ~M-x robe-start~
    #+BEGIN_SRC emacs-lisp
      (use-package robe
        :defer t
        :ensure t
...
*** reasonml lang server config
   ...
   1. *Run* ~ln -sf ~/w_emacs_dotfiles/reasonml/bin/reason-language-server /usr/local/bin~
   #+BEGIN_SRC emacs-lisp
